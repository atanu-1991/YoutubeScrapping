...
logging.basicConfig(filename='scrapper.log', filemode='w', level=logging.DEBUG, format='[ %(asctime)s ] %(lineno)d %(filename)s - %(levelname)s - %(message)s')
...
def image(path):
    '''
    This function is used to download thumbnail image
    :param path: video_id
    :return: NONE redirect to the /list_channel
    '''
    try:
